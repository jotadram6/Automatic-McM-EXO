...
RequestsToCloneEMu=ListOfRequests('EXO-RunIIWinter15pLHE-00001','EXO-RunIIWinter15pLHE-00038')
RequestsToCloneETau=ListOfRequests('EXO-RunIIWinter15pLHE-01813','EXO-RunIIWinter15pLHE-01850')
RequestsToCloneMuTau=ListOfRequests('EXO-RunIIWinter15pLHE-01851','EXO-RunIIWinter15pLHE-01888')

for request_prepid_to_clone in RequestsToCloneEMu:
...
for request_prepid_to_clone in RequestsToCloneETau:
    request = mcm.get('requests', request_prepid_to_clone)
    request[u'dataset_name'] = request[u'dataset_name'].replace("TuneCUETP8M1","TuneCP5")
    for key in modifications18: request[key] = modifications18[key]
    clone_answer = mcm.clone_request(request)
...
for request_prepid_to_clone in RequestsToCloneMuTau:
    request = mcm.get('requests', request_prepid_to_clone)
    request[u'dataset_name'] = request[u'dataset_name'].replace("TuneCUETP8M1","TuneCP5")
    for key in modifications18: request[key] = modifications18[key]
    clone_answer = mcm.clone_request(request)
